...
### 🟠 P2 — Concurrence & ressources — **0 ouvert sur 5** au 2026-09-09 (était 1 partiellement ouvert au 2026-09-08, 2 sur 5 au 2026-09-01)

*Rien ne reste. La moitié laissée par CONC-1 — `snapshot`/`cancel_job` lisant l'état d'un lot sans `_lock` — s'est fermée le 2026-09-09. Ce qui manquait n'était pas l'atomicité, que le GIL donne déjà champ par champ, mais la COMPOSITION : `snapshot` lit sept champs à la suite et le worker pouvait écrire entre deux.*

*Hors périmètre de CONC-1 et toujours vrai : un process tué emporte le lot sans laisser de trace (cf. `pilotage/CONC-2.md`, `abandonné`). Le correctif du journal A3 du 2026-09-08 ne change rien là-dessus — il inscrit les passes RATÉES, pas les lots que le process emporte avec lui.*
...
- ✅ **FERMÉ** (CONC-1, 2026-09-08 : la coupe passe entre le crop et le resize — et non « au dict de cache » comme le disait le correctif proposé ci-dessous, qui aurait produit un accès APRÈS FERMETURE, le `crop` lisant l'image partagée que `_open_image` remplace. Mesuré : 85 à 94 % du temps sortent de la sérialisation. Le TTL est porté par une minuterie d'INACTIVITÉ et non par un fil de fond, `TTL_MASTER_CROP`) — ~~**Verrou trop large dans `region_crop_png`.**~~ Le `with _crop_lock` englobe crop + resize LANCZOS + encodage PNG ([pipeline/ocr.py:144-167](pipeline/ocr.py#L144-L167)) : sous le threadpool FastAPI, **tous** les crops (vignettes de recherche incluses) sont sérialisés, encodage compris. Le cache garde aussi un TIFF (potentiellement des dizaines de Mo) ouvert indéfiniment, sans TTL ni fermeture à l'arrêt. **Correctif** : ne verrouiller que l'accès au dict de cache ; libérer/expirer l'image.

- ✅ **FERMÉ** (deux temps. **CONC-1, 2026-09-08** : registre borné en NOMBRE — `JOBS_CONSERVES`, lots terminés seulement — et son énumération placée sous le même verrou que la purge ; annulation devenue PRÉEMPTIVE par un interrupteur par fil, Kumiko entre deux tranches d'attente et l'OCR entre deux régions, 300 s ramenées à une demi-seconde. **Le sous-processus, lui, n'a jamais été orphelin** : `subprocess.run(timeout=…)` le tuait déjà dans ses deux clauses `except` — c'est le passage à `Popen`, nécessaire pour devenir interruptible, qui prend cette charge). **Puis le 2026-09-09** : `_lock` couvre désormais l'état mutable de chaque lot et plus seulement le registre — les sept écritures du worker le prennent, `snapshot` et `cancel_job` aussi. `snapshot` s'est DÉDOUBLÉ (`_snapshot_nu`, verrou tenu par l'appelant) parce qu'`all_jobs` l'appelle sous le verrou et qu'un `Lock` n'est pas ré-entrant : le nom dit qui tient quoi, là où un `RLock` aurait rendu légal l'ordre qu'on n'a pas réfléchi. Un aliasing s'est trouvé en chemin — `snapshot` rendait la liste VIVANTE des erreurs, défaut qui vaut même sur un seul fil. **Ce que le verrou ne ferme pas est écrit** : annuler à l'instant où le lot finit donne un lot `annule` avec `done == total`, et aucun verrou ne l'empêcherait — le travail était fait, et quelqu'un a bien cliqué Annuler) — ~~**Job : synchronisation implicite et registre non purgé.**~~ Le worker écrit `current/done/status/errors` pendant que `snapshot`/`cancel_job` les lisent **sans `_lock`** ([pipeline/jobs.py:48-64](pipeline/jobs.py#L48-L64) vs [pipeline/jobs.py:67-99](pipeline/jobs.py#L67-L99)) : ça « marche » grâce au GIL mais un `snapshot` peut voir un état composite incohérent. L'**annulation n'est pas préemptive** (testée seulement entre planches/passes — un Kumiko de 300 s ou un OCR de grosse planche ne s'interrompt pas, et le subprocess Kumiko n'est pas tué). Le registre `_jobs` **grossit indéfiniment** (fuite mémoire lente sur process long-vécu).
