...
- **B5 — `_migrate` ne lit jamais `user_version`** ([database.py:131-136](database.py#L131-L136)) : idempotent *par chance* (garde par présence de colonne), mais aucun *gating* par version ni garde anti-downgrade ; le pattern documenté « incrémenter `SCHEMA_VERSION` + ajouter une étape » sera **faux dès la 1re migration non détectable par colonne** (backfill, `UPDATE`). **Majeur (dette).**
