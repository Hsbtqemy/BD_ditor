...
| `/administration` | `administration.html` | `administration.js` | **Administration** (UX-10) : ce qui porte sur l'INSTANCE et non sur un album — panneaux **🏷️ Version servie** (INFRA-10, réservé aux administrateurs), **👥 Accès aux collections** (AUTH-3 — qui entre, à quel niveau, et s'il peut EXPORTER, une case à côté du niveau depuis DROIT-2 ; ce que la collection EST a déménagé dans la Bibliothèque avec COL-2), **👥 Comptes et groupes** (AUTH-12, qui remplace la vue des comptes d'AUTH-7 : une liste et une fiche, axes Comptes · Groupes · Collections, « À regarder » en tête ; il LIT l'annuaire par `GET /api/comptes-et-groupes` et ne modifie que la nature d'un compte) et **🩺 Moteurs** (SANTE-1). Aucune garde d'écran : chaque bloc pose sa propre question d'autorisation, jamais le contenant (leçon AUTH-4) |
