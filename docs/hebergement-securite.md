...
| ✅ **Corrigé** | **Aucune Content-Security-Policy** | `main.py` (`_csp`) | Était : aucun en-tête, donc rien pour amortir un XSS qui passerait l'échappement, ni contre le clickjacking. **Corrigé (SEC-2)** : `script-src 'self'` sans `'unsafe-inline'` — les surfaces n'ont aucun script inline, la politique ne coûte donc rien —, plus `object-src`/`base-uri`/`frame-ancestors` fermés. `/docs` et `/redoc` reçoivent une politique DISTINCTE (CDN autorisé, principes gardés) plutôt qu'une exemption. |
