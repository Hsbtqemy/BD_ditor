...
from socle import (  # noqa: F401  (ré-export : `main.X` reste un nom valide)
    _annotation_for_region, _ensure_tags, _get_album, _get_planche, _get_region,
    _csv_response, _csv_safe, _norm_tag, _refuser_si_verrouillee, _row, _rows,
    _sans_accents, _validate_parent,
    db, portee_courante,
)
import undo
...
# ARCH-1 — les domaines sortis de ce fichier. `include_router` les rend indiscernables
# de routes déclarées ici : même chemins, même place dans `app.routes`, donc les trois
# cliquets (autorisation, sorties d'identité, CSP) continuent de les voir.
from routes import recherche as _routes_recherche  # noqa: E402  (après `app`)
