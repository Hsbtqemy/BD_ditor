...
**Arrêté sur** — 2026-09-22, `644d090` : **« Comptes et groupes » prend une hauteur fixe et toute la largeur**, demandé par Hugo sur la recette. Le cadre suivait la longueur de la liste et plafonnait à 80rem ; il prend désormais la hauteur de la fenêtre, jamais moins de 28rem, la liste et la fiche y défilent chacune seule, et la fiche met ses deux sections côte à côte quand elle a la place. Rien ne change sous 40em. Trois tests navigateur, chacun tombé sur son mutant ; sur copie, les tests du bloc (20), l'accessibilité, le reflow, la police et « Qui entre » (133) et la suite par défaut (1385) verts. La passe `comptes-et-groupes`, pas encore jouée, gagne trois cases pour cette forme et passe à 33. Aucune case de ce `Reste` ne se coche avec lui.

Juste avant, `9273d8a` (2026-09-18) : **cinq défauts de l'écran réparés, dont un droit accordé par accident.** Quatre viennent des captures de Hugo — les actes liés chapeautés au lieu d'une barre qu'on prenait pour un curseur, l'avertissement qui nomme enfin son acte, les notes permanentes rangées en un bloc, et « ⚠ À regarder » qui cesse d'être allumé quand il n'annonce rien ; le cinquième est la moitié écran du droit d'export, dont la moitié serveur est ci-dessous. Éprouvé sur une copie des DEUX lots réunis : suite par défaut entière verte (1 385) et passe e2e entière verte (282), les trois tests neufs vérifiés présents dans la collecte. Détail dans « Ce que la relecture de l'écran a trouvé ».

Juste avant, `b9f60fe` : **la passe de revue d'avant la fusion a trouvé un droit accordé par accident, et une route qu'on payait pour rien.** Rétrograder un propriétaire lui accordait l'export (`3b44001`, moitié serveur d'une réparation qui en compte deux) ; `GET …/annuaire/verifier`, sans aucun appelant, est retirée (`b9f60fe`). Les deux sont décrits dans « Un droit accordé par accident, et la route qu'on payait pour rien », en bas de fiche.
...
Plus tôt, `3eb52e8` : **l'étape 2 est construite et éprouvée.** « 👥 Comptes et groupes » remplace « 👤 Comptes vus » dans l'Administration : une liste et une fiche, axes Comptes · Groupes · Collections, filtre, tri par nom ou par récence, « ⚠ À regarder » en tête, l'axe, la sélection et le tri dans l'adresse. Il lit la route `GET /api/comptes-et-groupes` construite par `AUTH-6` et ne modifie que la nature d'un compte. Éprouvé : 66 cas de logique pure sous Node, 15 tests navigateur dont un sur la vraie route avec la doublure de l'annuaire, l'audit axe sur trois fiches dans les deux thèmes, le reflow à 320 px sur trois états ; mutants tués : 25 sur 25 pour la logique pure, 20 sur 20 pour l'écran, 2 sur 2 pour le style ; suite par défaut entière (1350) et passe e2e entière (268) vertes sur copie. La passe de QA `pilotage/qa/comptes-et-groupes.md` est écrite, pas jouée. Ce que la construction a trouvé est dans la section « L'étape 2 construite ». Reste l'étape 3 — les accès dans la fiche de la collection — et, à ce moment-là, les fiches de l'étape 2 passent aux actes servis par le serveur. Avant elle : `abc3dc0` (2026-09-16), les quatre défauts de la zone « Défauts trouvés en lisant ».
...
- [x] **Aucun texte d'écran ne dit « principal » ni « genre »** — attendu : `templates/` et `static/` n'emploient, pour ce qu'ils montrent, que le lexique de la décision 7 — compte, personne, groupe, accès, propriétaire, administrateur. Aujourd'hui la ligne d'ajout d'un accès demande « Utilisateur ou groupe ? », ce qui est juste, mais ses messages de refus et ses attributs lisibles sont à relire — **le bloc « 👥 Comptes et groupes » tient le lexique depuis `3eb52e8`** (la passe de QA le relit) ; le panneau « 👥 Accès aux collections » le prendra à l'étape 3, en déménageant (section « Ce que la construction a tranché »), et la case se coche alors — **fait le 2026-09-18, `6ea6b60`** : « Qui entre » a pris le lexique en déménageant (« Compte ou groupe ? », « Le groupe X entre dans… », « compte » et « groupe » dits au lecteur d'écran), et `test_aucun_texte_d_ecran_ne_dit_principal_genre_ni_utilisateur` le MESURE sur ce que les deux blocs MONTRENT — texte rendu et attributs lisibles —, jamais sur le source. Une occurrence reste dans `templates/index.html`, écrite ici pour qu'on ne la cherche pas deux fois : « Utilisateur » y nomme l'identifiant d'un service TIERS, la connexion ShareDocs, et non un compte de l'instance
...
- [x] **L'écran ne connaît aucun niveau en dur** — attendu : la liste des actes, leur ordre et leurs liaisons lui sont DÉCRITS par le serveur, depuis l'endroit où `autorisation.py` les tranche ; ajouter un niveau ou une case hors rang ne touche pas au JavaScript de l'écran. C'est la condition pour que la décision d'`AUTH-10` arrive sans refaire l'écran, et la même règle que la garde : une seule source, côté serveur. **La moitié serveur est faite le 2026-09-17 (`3a30843`, `4db9e03`)** : `GET /api/droits` sert l'échelle, les actes avec leurs liaisons et l'avertissement, et ce qui est hors rang, depuis `autorisation.ACTES` et `HORS_RANG` ; aucune garde ne lit cette table, et chaque acte y est JOUÉ au niveau qu'elle lui donne (section « L'étape 3 côté serveur »). La case se coche avec l'écran qui la lit — **la moitié écran est faite le 2026-09-18, `6ea6b60`** : `static/lib/droits.js` déduit tout de cette description — les colonnes et leur ordre, l'état et la modifiabilité d'une case, le niveau envoyé après un geste, la phrase en toutes lettres — et un test statique lui interdit tout code de niveau ou d'acte hors commentaire. Éprouvé sur la description d'aujourd'hui ET sur deux issues hypothétiques d'`AUTH-10` — un cran `contribution` de plus, une case hors rang pour le vocabulaire : 42 cas, 20 mutants sur 20 tués
- [x] **Les libellés des actes sont validés par Hugo** — attendu : les six libellés de `autorisation.ACTES` (« lire », « annoter », « structurer le corpus », « gérer le vocabulaire », « lancer des lots », « décider qui entre »), celui d'« exporter » et l'avertissement « Supprimer un album efface ses images et ne se rattrape pas. » sont retenus ou remplacés par Hugo, avant que l'écran de l'étape 3 les montre. Posés PROVISOIRES le 2026-09-17 (`3a30843`), repris du tableau de la section « La finesse des droits » ; la coordination les lui soumet — **validés par Hugo le 2026-09-17, avec deux remplacements recommandés par la coordination** : « structurer le corpus » devient **« organiser les albums »**, et « lancer des lots » devient **« lancer la reconnaissance automatique »**. « lire », « annoter », « gérer le vocabulaire », « décider qui entre », « exporter » et l'avertissement sont retenus tels quels. Les CODES des actes ne changent pas (`structurer`, `lots`). Appliqué dans `autorisation.ACTES` (`0019205`), que `GET /api/droits` sert
- [x] **Les fiches de « 👥 Comptes et groupes » passent aux actes servis par le serveur** — attendu : à l'étape 3, là où la fiche d'un compte, d'un groupe ou d'une collection dit aujourd'hui un NIVEAU (« lecture », « écriture », « propriétaire »), elle dit les actes ouverts, lus dans la même description que l'écran d'attribution ; `static/lib/comptes.js` perd sa table de libellés de niveaux, et aucune correspondance niveau → actes n'est écrite dans le JavaScript. L'étape 2 affiche le niveau seul, décision de la coordination du 2026-09-17 ; les deux étapes partent dans la même fusion, donc la production ne verra jamais les deux écrans se contredire — **fait le 2026-09-18, `6ea6b60`** : les trois fiches disent les actes (« tous les actes, dont décider qui entre » ; « lire, annoter · organiser les albums · … ») par le même module que le tableau ; `static/lib/comptes.js` perd sa table de libellés de niveaux, et son cas du niveau inconnu est repris par `tests/js/droits.test.js`. Sans description lisible, la fiche retombe sur le niveau tel quel plutôt que de se vider
...
- [x] **Les listes se trient par NOM ou par RÉCENCE** — demandé par Hugo le 2026-09-16 en retenant la forme 1 : « Il manque seulement un mode de tri je pense (date et alphabet peut-être ?) ». Ce que la seconde version de la maquette en a fait, repris comme attendu : **« A → Z »** sur le nom LU — le nom lisible d'une personne, à défaut son login ; le nom d'un groupe ou d'une collection ; **« Récents d'abord »** par date — la dernière venue pour une personne, la dernière venue d'un de ses membres pour un groupe, la dernière modification pour une collection ; ce qui n'a pas de date passe à la fin ; « À regarder » reste en tête, hors tri. Attendu : sur chacun des trois axes, les deux tris rendent l'ordre décrit, un objet sans date est en fin de liste, et le tri choisi survit au changement d'axe et à l'adresse. **Deux de ces dates n'existent pas encore, et la troisième est approchée** — la case ne se coche pas sans elles : (1) la dernière modification d'une COLLECTION n'est pas en base : `collection` ne porte qu'une `date_creation`, et modifier ses descripteurs n'écrit rien au journal — c'est le défaut « Modifier une collection ne laisse aucune trace » qui la rendrait lisible ; ranger ou sortir un album n'y écrit rien non plus, et la définition de « modification » devra dire si cela compte — **définition tranchée par Hugo le 2026-09-17 : la description ou les accès, jamais les albums** (section « Ce que la construction a tranché ») ; (2) la dernière venue d'un membre suppose de connaître les MEMBRES d'un groupe, que l'application ne lit pas aujourd'hui — la lecture de l'annuaire d'`AUTH-6` ; (3) la dernière venue d'une PERSONNE existe (`utilisateur.derniere_vue`), mais seulement pour qui est venu, et à l'heure près : elle n'est réécrite qu'une fois par heure par login. Un compte jamais venu n'a pas de date, et se range donc en fin de liste — **fait le 2026-09-17, `3eb52e8`**, sur les trois dates que la route rend désormais : la dernière venue d'un compte, la plus récente venue d'un membre pour un groupe (membres lus dans l'annuaire), la dernière modification de la description ou des accès pour une collection. Un objet sans date — ou à la date illisible — passe à la fin, et deux ex æquo se départagent par nom. Le tri est gardé dans l'adresse et survit au changement d'axe comme au rechargement. Éprouvé par les tables de `tests/js/comptes.test.js` et par `test_le_tri_survit_au_changement_d_axe_et_au_rechargement` ; mutants tués : sans date en tête, ordre croissant, collation absente, tri remis à zéro au changement d'axe — et « ex æquo non départagés », qui a d'abord SURVÉCU parce que le tri du moteur est stable et que le jeu recevait les ex æquo déjà dans l'ordre ; le cas en ordre inverse l'a fait tomber

### L'étape 2 — le bloc « 👥 Comptes et groupes »
- [x] **La passe « Comptes et groupes » est jouée sur la recette** — attendu : `pilotage/qa/comptes-et-groupes.md` jouée par Hugo sur la pile de recette reconstruite sur `3eb52e8` ou après, avec l'annuaire réellement lu (et non sa doublure), et ses cases cochées par lui. Elle joue les gestes de l'administrateur que la maquette faisait jouer d'abord ; l'annuaire en panne et le regroupement de « À regarder » sont éprouvés par `tests/test_e2e_comptes_groupes.py`, la recette ne permettant ni l'un ni l'autre sans toucher la pile — **jouée par Hugo le 2026-09-23, 34 cases sur 34, aucune laissée**, sur la recette servant `c9901b7` puis `c888740`. L'annuaire y était RÉELLEMENT lu : mesuré sur la pile le jour même, `etat: lu`, 11 comptes et 7 groupes, et le bloc affiche son heure de lecture. **La passe a trouvé un défaut, réparé dans la passe même** (`c888740`) : le cadre se comptait depuis le haut de la PAGE, débordait d'une fenêtre de 500 px, et la liste cessait alors de défiler faute d'en manquer. La recette a été reconstruite en cours de passe, et les deux cases neuves jouées sur la version réparée. Quatre cases sont nées pendant la passe — trois pour la forme du 2026-09-22, une quatrième pour cette réparation ; elle en comptait 30 à l'ouverture
- [ ] **« Départ » retrouve l'étape d'archivage quand elle existe** — attendu : la partie « Départ » de la fiche d'un compte propose de le ranger dans un groupe qui lui ferme l'application, le jour où la règle `deny` d'`AUTH-7` est posée ET éprouvée (sa case « Une règle `deny` en tête d'`access_control` refuse bien l'accès »). La maquette l'écrivait déjà ; l'écran ne le dit pas, parce que ce serait faux aujourd'hui (section « L'étape 2 construite »)

### L'étape 3 — « Qui entre » dans la Bibliothèque
- [x] **La passe « Qui entre » est jouée sur la recette** — attendu : `pilotage/qa/qui-entre.md` jouée par Hugo sur la pile de recette reconstruite sur `6ea6b60` ou après, avec l'annuaire réellement lu (et non sa doublure), et ses cases cochées par lui. Elle joue les gestes du propriétaire que la maquette faisait jouer d'abord — faire entrer un cours, cocher ses actes, retirer un accès — puis ce que l'Administration en montre, et finit par la remise en état. L'annuaire en panne et l'absence des mots du modèle à l'écran ne s'y jouent pas : ils sont éprouvés par `tests/test_e2e_qui_entre.py`, la recette ne permettant pas le premier sans toucher la pile — **jouée par Hugo le 2026-09-18**, 23 cases sur 23, aucun défaut, sur la recette servant `6ea6b60`. L'annuaire y était RÉELLEMENT lu et non sa doublure : mesuré le matin même (`AUTH-6`, `etat: lu`, 11 comptes et 7 groupes), et sa capture de « Comptes et groupes » l'après-midi affiche « Annuaire lu à 15:20 ». **Jouée AVANT les correctifs du même jour** (`9273d8a` : le trait bleu devenu chapeau de colonnes, l'avertissement rattaché à son acte, les messages rangés) : une seule case touche ce qu'ils ont changé — le refus du dernier propriétaire —, et son énoncé tient toujours. Ces correctifs sont prouvés par l'e2e (282 verts), pas par cette passe
...
- [ ] **La documentation d'usage ne décrit plus ce parcours** — `docs/modele-et-droits.md` dit qu'un retrait de groupe ferme la porte « immédiatement », ce que la case « Le délai d'un RETRAIT de groupe est mesuré » doit trancher avant qu'on l'écrive autrement ; aucun document ne dit qu'un compte doit être dans `lldap_admin` pour créer des comptes, ni que « Mot de passe oublié ? » échoue pour ces comptes ; aucun ne décrit le départ d'une personne comme une suite ordonnée de gestes — l'ÉCRAN le fait depuis l'étape 2, dans la partie « Départ » d'une fiche de compte, les documents non. Attendu : les deux documents d'usage décrivent le parcours tel qu'il sera après les décisions ci-dessus — c'est pourquoi cette case attend, plutôt que de corriger aujourd'hui un texte qui changera. **Deux griefs sont tombés le 2026-09-18, et pas de la même façon.** `docs/guide-utilisateur.md` ne renvoie plus « Créer un compte, un groupe » au fichier des comptes du portail : il renvoie à l'annuaire et dit que le fichier n'est plus que le repli (`64566ae`). Et `docs/hebergement-securite.md`, qui rangeait l'octroi d'accès dans la Bibliothèque au temps où il vivait dans l'Administration, dit VRAI depuis que l'étape 3 l'y a ramené : personne ne l'a réparé, c'est le code qui est revenu là où le texte l'attendait. Une phrase juste par accident est à relire au prochain déménagement, pas à oublier
...
## L'étape 2 construite — 2026-09-17

Construite par la session de l'étape 2, sur le format de route accordé avec celle d'`AUTH-6` par la coordination (section « Ce que la construction a tranché »). Ce qui suit est ce que la construction a TROUVÉ, et que ni la maquette ni le format ne disaient.

**Une phrase de la maquette aurait été fausse à l'écran.** Sa partie « Départ » proposait de « ranger le compte dans le groupe archives : il ne peut plus se connecter ». Aucune règle `archives` n'existe — ni dans `deploy/`, ni dans la documentation d'exploitation —, la case d'`AUTH-7` qui l'éprouverait est ouverte, et LLDAP ne sait pas désactiver un compte. L'écran dit donc trois étapes vraies : retirer ses accès à son nom, le retirer de ses groupes dans l'annuaire, et ce qu'une suppression laisserait — une CONSÉQUENCE, jamais une recommandation, selon la doctrine du verdict d'`AUTH-7`. Et « ses actes perdraient leur auteur », première rédaction, était inexact : le journal est clé sur le login, les actes y restent, et c'est un compte recréé sous le même nom qui en hériterait.

**Un défaut que seul le navigateur montrait.** « Fin » dans la liste atterrissait dans le repli fermé des comptes de l'annuaire. La ligne visible se reconnaissait à son `offsetParent` non nul, or Chromium ne rend plus le contenu d'un `<details>` fermé en `display: none` : il y reste renseigné. Le filtre passe par l'état du repli.

**L'audit axe ne voyait pas les lignes de la liste**, et c'est un mutant de STYLE qui l'a montré : l'ambre brut en petit texte, 2,14:1, passait l'audit. Déplier « À regarder » poussait les lignes hors de la colonne défilante, et axe range en « incomplet », que l'audit ne lit pas, le contraste d'un texte qu'un conteneur défilant cache. Le test du bloc audite désormais dans une fenêtre assez haute pour tout montrer ; l'angle mort, qui peut toucher d'autres audits, est porté dans `A11Y-3`. Un second mutant de style, une grille trop large à 320 px, a d'abord survécu pour une raison inverse : il ne cassait rien, une seule colonne étant visible à la fois. Remplacé par une colonne qui déborde vraiment, il est tombé.

**Deux survivants de mutation, deux tests qui ne regardaient pas là.** Les ex æquo de « Récents d'abord » n'étaient départagés par rien sans qu'un test tombe : le tri du moteur est stable, et le jeu recevait déjà les ex æquo dans l'ordre. Et un repli de « À regarder » déplié se refermait au rechargement sans qu'un test tombe : aucun ne rechargeait après l'avoir déplié. Chacun a reçu son cas, et le mutant est tombé sur lui.

**Ce que le bloc laisse à d'autres fichiers**, pris par la coordination après le commit : `docs/guide-utilisateur.md` et `docs/modele-et-droits.md`, qui nomment « Comptes vus » ; `pilotage/qa/compte-collectif.md`, qui nomme les anciens libellés de nature (« Collectif (login partagé) », devenus « un login partagé ») ; et le retrait de `GET /api/comptes`, qui n'a plus de lecteur, par `AUTH-6` — fait le 2026-09-17 (`bfb9d32`, section « L'étape 3 côté serveur »).

## L'étape 3 côté serveur — 2026-09-17

Construite par la session d'`AUTH-6`, sur le cadrage accordé avec la coordination et les décisions de Hugo consignées dans « Ce que la construction a tranché ». L'écran de l'étape 3 reste à faire ; ce qui suit est ce qu'il trouvera servi, et ce que la construction a trouvé.

**Ce qui est servi** (`3a30843`). `GET /api/collections/{id}/annuaire` rend au PROPRIÉTAIRE de cette collection — et aux administrateurs — l'état de la lecture et les NOMS des groupes proposables, sans les groupes de rôle de l'annuaire ni ceux des administrateurs, avec les accès actuels et, pour chacun, ce que l'annuaire en dit (« trouvé », « inconnu », « non vérifié », « sans annuaire »). `GET /api/collections/{id}/annuaire/verifier` vérifie UN nom tapé, compte ou groupe, contre tout l'annuaire : aucune liste des comptes n'est servie. Un lecteur ou un membre en écriture reçoit un 403 nommé, un étranger un 404. `GET /api/droits` sert à tous la description des actes : elle n'est pas un secret, et c'est la même que « Mon compte » (`AUTH-9`) doit montrer à la personne (case « La personne lit ce qu'elle peut FAIRE »). Les gestes d'accès eux-mêmes — lister, accorder, retirer — ne lisent PAS l'annuaire : une panne ou un délai de trois secondes n'y ralentit rien, et un test l'exige. Les deux routes d'annuaire sont déclarées au cliquet d'`AUTH-5`, la vérification comme un ORACLE : elle laisse sonder l'existence d'un compte, ce que la décision de Hugo acceptait ; `GET /api/droits` est déclarée hors du périmètre de la portée, avec sa raison. La table des actes est déclarée dans `AUTH-10`, section « Une description des actes est posée à côté de `NIVEAUX` ».

**Le retrait de `GET /api/comptes`** (`bfb9d32`). Ses tests ne sont pas partis avec elle : les règles de la vue des comptes — réservée aux administrateurs, ce qu'un compte laisse, son verdict, ses reprises d'identité, et la limite déclarée sans annuaire — s'éprouvent sur `GET /api/comptes-et-groupes`, qui les sert. La raison de la réserve aux administrateurs est reportée dans la docstring de la vue qui la remplace.

**Un test éprouvait l'échelle en croyant éprouver les actes.** La table de vérité de `tests/test_droits.py` choisissait la question de `Portee` — `peut_lire`, `peut_ecrire`, `peut_administrer` — d'après le niveau que l'acte DÉCLARE : un acte déclaré trop haut ou trop bas y restait cohérent. Deux mutants l'ont montré en survivant : « décider qui entre » déclaré en écriture, et « lire » déclaré en écriture. Le premier aurait fait promettre à l'écran qu'un membre en écriture élargit le cercle, ce que le serveur refuse. Désormais chaque acte est JOUÉ par un geste qui le représente — la liste des albums, une annotation, un album créé, un tag créé, un lot sur une planche verrouillée (compté sans qu'aucun moteur tourne), un accès accordé — sous un membre de chaque niveau, et un acte ajouté sans geste fait échouer le test (`4db9e03`). Rejoués après le correctif, les deux mutants tombent sur ce test. **Sa limite est écrite dans sa docstring** : un geste par acte n'est pas le périmètre route par route d'`AUTH-10`.

**Éprouvé sur copie.** Suite par défaut entière verte à `bfb9d32` (1365). Harnais de 26 mutants : 20 sur l'étape 3 (la liste proposée, la vérification, les gardes, l'ordre, un geste d'accès qui lirait l'annuaire, la table des actes, les déclarations aux deux cliquets) et 6 sur le retrait, chacun joué contre `tests/test_collections_admin.py` SEUL pour prouver que les tests reportés mordent sur la nouvelle route — dont une déclaration survivante de `/api/comptes`, que le cliquet d'`AUTH-5` refuse. Premier passage : 23 tués ; D01 et D03 sont le trou ci-dessus ; le troisième survivant venait du harnais, qui jouait un mutant du semis contre un test incapable de voir une émission en MOINS, et il tombe contre le bon. 26 sur 26.

## L'étape 3 construite (écran) — 2026-09-18

Construite par la session de l'écran, sur le cadrage accordé avec la coordination et les décisions de Hugo du 2026-09-17 : forme (A), les accès en tête de la collection dépliée ; cartes sous 48em ; « Faire entrer » accorde en LECTURE, puis on coche — deux gestes. Déménagé, pas dupliqué : le panneau « 👥 Accès aux collections » quitte l'Administration dans le même commit, et la Bibliothèque devient adressable (`?collection=<id>`, `&groupe=<nom>`), ce qui donne enfin leur cible à « Régler qui entre » et à « Ouvrir une collection à ce groupe… ».

**L'écran ne connaît aucun niveau, aucun acte, aucun libellé.** Toute la logique vit dans `static/lib/droits.js`, qui ne lit que la description servie par `GET /api/droits` : les colonnes et leur ordre, l'état et la modifiabilité d'une case, le niveau à envoyer après un geste, la phrase en toutes lettres des fiches. Elle est éprouvée sur la description d'AUJOURD'HUI et sur deux descriptions HYPOTHÉTIQUES — un cran `contribution` de plus, une case hors rang pour le vocabulaire —, c'est-à-dire sur les deux issues qu'`AUTH-10` peut prendre. Un test statique interdit au module de porter un code de niveau ou d'acte hors de ses commentaires.

**Une case par CRAN, jamais par acte.** Les actes que le serveur accorde ensemble partagent une cellule, traversée d'une barre, et le nom accessible de la case unique nomme la ligne PUIS toutes les colonnes couvertes (`aria-labelledby`) : la liaison se voit, et elle s'entend. Les cocher séparément aurait promis une finesse que le modèle n'a pas, qu'un refus serait venu démentir.

**Deux vrais défauts de style, trouvés par le test de débordement une fois porté sur la Bibliothèque, et dont la cause est CONFIRMÉE par mutation** — retirer le correctif, seul, rejoue la mesure d'origine. À 768 px la page entière défilait (1 098 px pour 768) : les libellés réservés aux lecteurs d'écran, en position absolue, n'avaient aucun ancêtre positionné et sortaient du cadre défilant. À 320 px avec une police de 20, le corps de la page débordait (395 px) : la liste « Faire entrer » prenait la largeur de sa plus longue option. Le repli des en-têtes d'actes, ajouté au passage, ne répare RIEN — il rétrécit le tableau de 1 098 à 943 px, qui défile encore dans son cadre : c'est de la lecture, et son mutant survit à juste titre, seul survivant des 32.

**La suite par défaut a trouvé ce que la passe ciblée ne pouvait pas voir.** `test_e2e_masquage` exige que toute cible d'un `.hidden` se résolve en `$("#id")` ; la saisie libre de « Qui entre » est visée par CLASSE, une par collection dépliée. Elle est déclarée dans `NON_RESOLUS` avec sa raison, et le garde de feuille qui la rend effective (`display: inline-flex` l'emporterait sur la règle UA) est éprouvé DANS LES DEUX SENS par deux tests e2e — l'un l'exige cachée, l'autre visible. Déclarer sans cela aurait éteint le contrôle sans rien mettre à sa place.

**Et la passe e2e ENTIÈRE a trouvé un test resté pointé sur le panneau déménagé**, seule des trois mesures : `test_le_panneau_des_acces_declare_les_administrateurs` — la note d'`AUTH-4`, qui déclare que les administrateurs lisent toute collection sans figurer dans les accès — allait toujours sur `/administration` et attendait une collection qui ne s'y affiche plus. Il tombait sur le DÉCOR, pas sur la règle. Ni la passe ciblée ni le harnais ne le jouaient : le mutant de cette note visait l'autre branche, celle du participant qui ne voit pas « Qui entre ». La leçon n'est pas qu'il manquait un test, c'est que la relecture des tests à retargeter a cherché parmi les tests des ACCÈS, et que celui-ci vivait parmi ceux d'accessibilité. Porté dans la Bibliothèque sous son vrai nom (`test_qui_entre_declare_les_administrateurs`) ; le mutant qui retire `colAdminNote()` de « Qui entre » l'y fait tomber.
...
## Ce que la relecture de l'écran a trouvé — 2026-09-18

Quatre défauts vus par Hugo sur des captures, réparés le jour même (`9273d8a`). Le cinquième de la journée — un droit d'export accordé en rétrogradant — vient de la relecture croisée du périmètre par une session qui n'avait écrit aucune de ces lignes ; il est décrit dans « Un droit accordé par accident, et la route qu'on payait pour rien », plus bas, et sa moitié écran part dans le même commit. Ce qui suit est ce que les quatre ont appris, et qui ne tient pas dans un message de commit.

**Une intention structurelle rendue par un dessin perd contre le dessin.** La barre qui traversait la cellule des actes liés disait la liaison — et disait aussi, sans qu'on l'ait voulu, le vocabulaire d'un `input[type=range]` : deux pixels en travers, une case posée au milieu, on croit pouvoir la glisser. Remplacée par un CHAPEAU qui couvre les colonnes liées (`scope="colgroup"`, second rang d'en-têtes). Le gain n'est pas seulement qu'on ne s'y trompe plus : une structure de tableau s'ANNONCE au lecteur d'écran, un dessin ne s'annonce jamais. Écartée, la marque ⚠ dans l'en-tête de l'acte : elle redirait ce que la phrase et l'`aria-describedby` disent déjà, au prix d'une colonne dont la largeur est mesurée à 320 et 768 px.

**Un avertissement séparé de son acte était perdu À LA SOURCE, pas à l'affichage.** `BDDroits.avertissements()` ne rendait que des PHRASES : l'acte était jeté par la fonction, et l'écran n'avait rien à rattacher. Le réparer dans la feuille de style aurait été rattraper ce que le module avait perdu. Elle rend désormais l'acte avec sa phrase, et la case du cran qui accorde cet acte le cite en `aria-describedby` — ce qui est vrai, et non décoratif : cocher ce cran accorde bien cet acte. Un lecteur d'écran y gagne une information qu'il n'avait pas du tout. Le troisième défaut tombait alors presque seul : trois traitements visuels empilés en trois lignes ne disaient pas lequel comptait, et il n'en restait que deux une fois l'avertissement rattaché — ils tiennent dans un bloc à filet unique, distinct de la ligne de message, qui vient du dernier GESTE.

**Une alerte allumée en permanence est une alerte perdue.** « ⚠ À regarder » s'affichait encadré d'ambre, titré et DÉPLIÉ même à zéro signal : le seul élément de la page conçu pour attraper l'œil était allumé tout le temps, pour dire « Rien à regarder ». C'est le mode d'échec le plus coûteux des quatre parce qu'il ne se remarque jamais — qui ouvre la page chaque semaine et voit toujours le même cadre cesse de le lire, et le jour où il y a vraiment un accès mort, le cadre a exactement la même apparence qu'un mois de rien. On perd le signal en l'ayant toujours montré. À zéro, l'alerte s'éteint donc entièrement (ni ⚠, ni ambre, ni cadre, repliée) et ne garde que son compte ; elle se rallume au premier signal. C'est la règle déjà tranchée pour le bandeau de portée vide, qui nomme les quatre situations mais ne se déplie d'office que pour la seule panne CERTAINE — et dont la fiche dit que n'avoir pas déplié sur un diagnostic non mesuré est ce qui a rendu un défaut discret plutôt que criant. **Deux précautions dans la garde, et chacune ferme une façon de la rendre inutile** : elle mesure les valeurs CALCULÉES (`getComputedStyle`) et non la présence d'une classe, sans quoi une règle disparue de la feuille laisserait le cadre allumé sous une garde verte ; et elle joue les DEUX états, sans quoi on pourrait éteindre l'alerte pour de bon — « elle ne crie jamais » est pire que « elle crie toujours », et tout aussi vert. L'ouverture, elle, se décide au premier rendu et aux changements d'état seulement : la forcer à chaque redessin rouvrirait sous la main le bloc qu'on vient de replier, la vue se rechargeant après chaque geste.

**Et ce que la RÉPARATION du cinquième a appris, qui vaut au-delà de lui : deux moitiés réparées en parallèle se rendent VERTES l'une l'autre.** La section d'en bas dit pourquoi aucune suite ne voyait NAÎTRE ce défaut ; ceci est l'autre moment, celui où on le répare. Le test d'écran relisait l'état final après la rétrogradation — donc la fermeture serveur l'aurait fait passer même si l'omission ne se déclenchait jamais ; le test serveur, s'il était passé par l'écran, serait passé que l'écran envoie le champ ou non. Chacun aurait mesuré la moitié de l'AUTRE en croyant mesurer la sienne, et les deux suites auraient été vertes — c'est-à-dire exactement l'état dans lequel ce défaut est né. Remède appliqué des deux côtés : chaque test mesure SA moitié au plus près du geste — la charge utile du `PUT` pour l'écran, un `PUT` posé sans écran pour le serveur —, et garde une seconde assertion pour la paire. C'est la troisième forme prise le même jour par une même propriété : **ce qui vit exactement sur la couture entre deux moitiés est invisible des deux côtés**. Les deux autres formes : le défaut lui-même ; et le fait qu'aucun test de la suite par défaut ne lit le tableau de « Qui entre », de sorte qu'un vert y serait un décor — ce qui a commandé l'ordre de la fenêtre, les deux lots réunis puis la suite entière puis l'e2e, et non l'e2e seul.
