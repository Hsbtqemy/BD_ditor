...
**Arrêté sur** — 2026-09-16, `749ceb8` : les premiers défauts de la zone « Défauts trouvés en lisant » sont réparés en code. Modifier une collection laisse une trace au journal, et `tools/gerer_collections.py` a les gardes et le journal des routes. Hugo a tranché pour la collection créée par un administrateur : elle reste sans propriétaire, et c'est l'écran qui dira vrai (option B). Restent, en code : ce message de création et la modale d'album, dans `static/corpus.js`, gelé par une passe voisine au moment de l'arrêt ; puis le mutant « journaliser tous les champs » de la route, qui n'a pas pu être joué pour la même raison.
