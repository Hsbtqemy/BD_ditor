...
- [ ] **Une collection créée par un administrateur n'a AUCUN propriétaire, et l'écran lui dit le contraire** — `create_collection` n'inscrit de propriétaire que pour un compte qui n'est pas administrateur, et la Bibliothèque affiche à tous « « X » créée — vous en êtes propriétaire ». `CLAUDE.md` dit « zéro propriétaire » interdit en base ; seuls le RETRAIT et la RÉTROGRADATION du dernier propriétaire sont refusés. Attendu : le message dit vrai pour un administrateur, et la règle est soit tenue à la création, soit écrite comme limite
- [ ] **Modifier une collection ne laisse aucune trace** — `PATCH /api/collections/{id}` n'écrit rien au journal A3, alors que les accès (`lien`, `delien`), la création et la suppression y sont. Changer le référent, le régime de diffusion ou la base légale n'a donc ni auteur ni date. Attendu : un événement par modification, avec avant et après
...
- [ ] **`tools/gerer_collections.py` défait ce que les routes gardent** — `retirer` et `supprimer` n'ont ni la garde « aucun album hors collection » (409 à l'écran) ni l'écriture au journal. Attendu : les mêmes gardes que les routes, ou la limite écrite dans l'aide de l'outil
...
tiroirs sous le seuil (`UX-7`, piège à focus éprouvé).
