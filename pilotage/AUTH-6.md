...
**Arrêté sur** — 2026-09-18, `4d00cbd` : le scénario entier d'une collection qui perd son
propriétaire est JOUÉ, et rejouable — cinq temps par la route, cinq mutants tués, suite
par défaut entière verte sur une copie (1367). La lecture de l'annuaire, elle, est
construite depuis `fb80fec` (étape 1 d'AUTH-12) et mesurée sur la recette le 2026-09-18.
...
- [x] Ce qu'une collection devient quand son unique propriétaire perd son groupe : la base refuse le zéro-propriétaire par un 409, mais ce refus porte sur une SUPPRESSION d'accès, pas sur une appartenance qui s'évapore côté annuaire. **Tranché le 2026-09-09 : `bd-admins` comme recours ne suffit pas, les collections orphelines doivent être SIGNALÉES.** Attendu : un écran ou un contrôle liste les collections dont aucun propriétaire déclaré n'a d'appartenance vivante, et un test joue le scénario entier — le propriétaire perd son groupe, la collection apparaît dans la liste, un administrateur réattribue la propriété. **BLOQUÉE derrière la lecture d'annuaire** (case « L'application LIT l'annuaire », zone suivante), noté le 2026-09-10 : l'attendu suppose de savoir ce qui EXISTE dans LLDAP, et l'application n'en lit rien aujourd'hui — mesuré, pas une ligne. Tant que cette décision n'est pas exécutée, cette case n'est pas actionnable, et elle se lisait comme si elle l'était. **Et l'écran qui listera ces collections vit en Administration**, par la frontière tranchée dans `COL-2` le même jour : une propriété est un ACCÈS, donc « qui entre » — la Bibliothèque prend les descripteurs, pas les accès. **Débloquée côté serveur le 2026-09-17 (`fb80fec`)** : la vue émet `proprietaire_absent` quand aucun propriétaire n'est VIVANT — un compte absent de l'annuaire, un groupe absent ou sans membre —, et `sans_proprietaire`, hors collection de repli — `test_un_proprietaire_qui_n_existe_plus_ou_un_groupe_vide_ne_tient_pas_une_collection`. **L'écran marque ces collections depuis le 2026-09-17 (`3eb52e8`, AUTH-12)**, « sans propriétaire » comme « propriétaire absent de l'annuaire ». Reste le test du scénario ENTIER (le groupe perdu, la collection qui apparaît, la propriété réattribuée), qu'aucun test ne joue encore. **JOUÉ le 2026-09-18 (`4d00cbd`), et la case se ferme là-dessus.** `test_une_collection_perd_son_proprietaire_puis_le_retrouve` joue les cinq temps par la route : un groupe propriétaire avec des membres et rien à signaler ; ses membres le quittent dans l'annuaire ; la collection apparaît en `proprietaire_absent`, dans la vue ET dans « À regarder » ; un administrateur réattribue ; le signal s'ÉTEINT. Le cinquième temps est le plus important — un test qui s'arrêterait à l'apparition prouverait qu'on sait crier, jamais qu'on sait se taire —, et le silence s'y affirme AVANT la relève pour que la mutation le fasse rougir lui. La seconde forme de la perte est jouée à côté : le groupe qui DISPARAÎT, d'où la collection orpheline ET l'accès mort, deux signaux que deux gestes distincts réparent. **Ce qui rend le scénario rejouable** : la doublure du dépôt est un décor FIXE, donc insuffisante pour une appartenance qui change entre deux lectures — le test en copie le contenu dans son dossier temporaire et réécrit cette copie, laissant intact le fichier partagé que d'autres sessions lisent, chaque lecture passant par le même analyseur que LLDAP. **Et chaque temps qui conclut exige `annuaire.etat == "lu"`** : sans quoi les deux temps qui concluent au SILENCE passeraient aussi sur un annuaire muet, qui ne déclare jamais rien mort — le test dirait « aucun signal » en ayant mesuré « je n'ai pas pu regarder », la maladie d'ARCH-2. Éprouvé par CINQ mutants, tous tués sur l'assertion visée : ne jamais émettre, émettre toujours, juger un groupe sur sa seule EXISTENCE en ignorant ses membres, empêcher le signal de s'éteindre, couper la remontée dans « À regarder ». Le troisième est celui qui prouve que le scénario porte sur l'APPARTENANCE et non sur le nom. **Ce que le test ne prouve pas** : il joue sur la doublure, pas sur un LLDAP réel — l'analyseur est le même, le serveur ne l'est pas ; et le constat de la recette reste celui du 2026-09-18, trois collections tenues par des propriétaires vivants.
