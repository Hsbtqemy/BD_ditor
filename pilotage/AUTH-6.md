...
- [ ] **Un compte COLLECTIF est-il prévu, et sous quelle forme ?** — non décidé au 2026-09-07, et c'est cette fiche qui tranche : la question est venue d'`AUTH-9` (« donner des noms historiques aux différentes personnes ayant travaillé sous cette identité collective »), qui a mesuré ce qu'elle coûte sans pouvoir y répondre. Attendu : la réponse distingue le SUCCESSIF du SIMULTANÉ, parce qu'un seul des deux est atteignable. Un login passé de main en main laisse déjà une coupe datée (`_identite_reprise`, AUTH-7) qu'un registre pourrait APPLIQUER ; plusieurs personnes tapant sous le même login ne laissent rien, et un registre y serait une déclaration, jamais une attribution. Aucun compte collectif n'est en service — `utilisateur` ne contient que `chercheur` et `stagiaire`, mesuré le 2026-09-07
