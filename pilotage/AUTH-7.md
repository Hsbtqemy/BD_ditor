...
- [ ] **Mais la RESTRICTION à `bd-admins` n'est pas prouvée, et le 302 ne la prouve pas** : il dit qu'un anonyme est renvoyé au portail, pas qu'un compte ordinaire sera refusé. Éprouver un seul versant ne distingue pas une règle juste d'un refus universel — c'est la leçon d'`INFRA-8`, dont l'arbitrage du facteur n'a été établi qu'en essayant `chercheur` ET `stagiaire`. À faire avec deux comptes réels une fois le proxy posé : `chercheur` entre, `stagiaire` est refusé
- [ ] Le bloc nginx de l'hôte pour `annuaire.edito-revue.fr` vers `127.0.0.1:8080`, son certificat, et `nginx -t` AVANT tout rechargement — un rechargement raté emporte tous les sites de la machine, pas seulement celui-ci
