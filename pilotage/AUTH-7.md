...
- [ ] **L'étiquette `lldap/lldap:stable` est FLOTTANTE**, et le schéma vient de migrer jusqu'à 11 sur une base neuve. C'est la leçon d'`INFRA-9` appliquée au service voisin : dès qu'une base a migré, redescendre l'étiquette n'est plus un retour arrière, et une flottante laisse l'instance vieillir pendant que le déploiement se croit à jour. À épingler sur la version que `stable` vient réellement de tirer — épingler avant le premier démarrage aurait demandé de deviner un numéro
