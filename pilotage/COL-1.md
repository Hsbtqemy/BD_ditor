...
- [ ] **Ce que la réparation REND POSSIBLE, et qui ne l'était pas** — trouvé en passe de revue, pas en écrivant. Une dimension GLOBALE re-rattachée à un domaine PRIVÉ devient désormais locale, et ses valeurs globales descendent avec elle : quelqu'un qui n'écrit que dans SA collection peut donc **capturer du vocabulaire partagé** et le retirer de la vue des autres. Avant, le même geste ne faisait rien (c'était le défaut). Le droit existait déjà — un terme global s'édite par qui écrit quelque part, « personne ne le possédant en propre » (`peut_ecrire_terme`) —, mais il n'avait pas cet effet. À trancher : laisser (c'est la conséquence cohérente de l'invariant, et c'est réversible en promouvant à nouveau), ou refuser par un 409 « privatiser du vocabulaire partagé demande de le dire ». Ne pas trancher, c'est choisir la première par défaut **Tranché pour le RATTACHEMENT le 2026-09-24 par Hugo, posé par `bef0b83` (AUTH-11)** : un rattachement ne déplace plus jamais une portée — une dimension globale sous un domaine privé est refusée (409, « rangez d'abord la dimension »), la capture par ce geste n'existe plus. **Elle reste possible par le RANGEMENT** (`PATCH …/lexique {collection_id: C}` sur un terme global), explicite mais silencieux sur ce qui descend : la question de cette case survit donc, déplacée, et une case d'`AUTH-11` la porte (« Ranger une dimension GLOBALE dans sa collection la retire à toutes les autres »)
- [x] **La contrainte v24 n'existait pas sur ces routes, et c'était mesuré** — 2026-09-06, sonde par TestClient. `PATCH /api/attributs/valeurs/{id}/lexique` avec `{collection_id: null}` répond **200** et laisse la valeur GLOBALE sous une dimension PRIVÉE. C'est exactement l'état que v24 interdit, atteint par la route sur laquelle COL-1 comptait. Attendu : un **409 qui nomme le parent**, comme les deux états interdits d'AUTH-3 — la personne a demandé « rends cette valeur globale », et l'honorer exigerait de promouvoir aussi sa dimension, ce qu'elle n'a pas demandé. **Fermée par `8dac360` le jour même** : c'est la réparation que décrit la case cochée « La contrainte v24 existe désormais sur les routes qui DÉPLACENT » — `_patch_lexique` (`socle.py`) refuse par un 409 qui nomme tous les ancêtres bloquants, sauf `promouvoir_parents: true`. Cette case-ci, le CONSTAT, était restée ouverte à côté de sa réparation ; relevé le 2026-09-11 en relisant la fiche contre le code
- [x] **Et un second chemin y mène, sans promotion du tout** : `PATCH /api/attributs/dimensions/{id}/domaine` rattache une dimension GLOBALE à un domaine PRIVÉ et ne re-dérive pas `collection_id` (200, mesuré). La création hérite (`routes/personnages.py`), le re-rattachement non — ce qui fuit alors n'est pas un mot mais le NOM DE L'AXE, c'est-à-dire la grille d'analyse d'une collection fermée. Celui-ci n'appelle aucune décision : le re-rattachement doit hériter comme la création. **Fait par `8dac360`, le 2026-09-06** : `patch_dimension_domaine` re-dérive désormais la portée du domaine visé (sa docstring : « v24 — la PORTÉE suit le rattachement »). Relevé le 2026-09-11 : la case était restée ouverte. **RENVERSÉ le 2026-09-24 (décision de Hugo, `bef0b83`, AUTH-11)** : ~~le re-rattachement doit hériter comme la création~~ — hériter au rattachement publiait une dimension privée placée sous un domaine global et retirait une dimension globale aux autres collections, deux effets silencieux. Le rattachement ne déplace plus aucune portée ; ce qui contredirait v24 est refusé par un 409. La fuite que cette case fermait reste fermée : une dimension globale ne peut plus être placée sous un domaine privé
- [ ] **Ce que ces deux trous coûtent vraiment, et pourquoi ils ne lèvent rien** — `lexique_resume` compte par APPARTENANCE seule (`database.py`, `WHERE 1=1 {scope}`, aucune jointure au parent), tandis que les lectures à plat filtrent le terme parent EN PLUS du terme. Le terme est donc COMPTÉ dans le « % défini » de tout le monde et MASQUÉ de leurs listes. C'est le désaccord précis que la migration v24 avait été écrite pour effacer, et il se réintroduit par la porte de devant. **État au 2026-09-11** : les deux portes sont fermées (`8dac360`, cases précédentes), donc le désaccord ne peut plus NAÎTRE par elles. Mais `lexique_resume` compte toujours par appartenance seule, sans jointure au parent : il tient tant qu'aucune route ne rouvre l'état « global sous un parent local », et rien ne le vérifie. Reste à décider si l'on s'en contente ou si le décompte doit filtrer le parent comme les listes
- [ ] Le domaine NAÎT toujours global : `DomaineIn` ne porte que `nom` (`socle.py`), la portée se pose ensuite par `PATCH /api/domaines/{id}/lexique`. Il existe donc une fenêtre où un domaine d'incubateur est visible de tous, entre sa création et son rangement. À trancher : fenêtre acceptable (le domaine n'a encore ni dimension ni valeur), ou `collection_id` à la création
...
qu'en la lisant. Trois d'entre elles disaient vrai — le filtre `clause_terme` de
`/api/tags`, la docstring de `LexiqueIn`, les quatre routes de promotion. La quatrième,
« à éprouver, y compris la contrainte v24 », a rendu l'inverse de ce qu'elle supposait :
**il n'y a pas de contrainte v24 à éprouver sur ces routes.**

**Post-scriptum du 2026-09-07, et il vise la méthode plutôt que le fond.** Ces références
étaient écrites en NUMÉROS DE LIGNE, et elles avaient toutes dérivé un jour plus tard —
`socle.py:300` et `socle.py:634` de soixante et cent lignes, déplacées par le commit
`8dac360` de cette fiche elle-même, qui a ajouté 111 lignes à `socle.py`. Une vérification
datée du 2026-09-06 concluant « la référence est encore exacte » était donc fausse le
lendemain, par son propre effet. Elles sont désormais écrites en NOMS : un symbole se
déplace avec le code, un numéro reste où le code n'est plus.
