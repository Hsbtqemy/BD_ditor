...
- [ ] **Créer puis partager — le trajet le plus courant est celui que la frontière coupe en deux.** `create_collection` fait de son créateur le propriétaire : on crée dans la Bibliothèque, et il faut aller dans l'Administration pour y faire entrer qui que ce soit. Attendu : le message qui suit la création dit où partager, avec le lien ; la section des accès dit où l'on crée et décrit ; et le trajet est éprouvé en le jouant, pas en lisant les deux écrans
