...
- [x] **Où vit l'écran, et la raison est écrite — tranché le 2026-09-10 : la BIBLIOTHÈQUE.** La frontière, qui est l'attendu de cette case et non la préférence : **_qui entre_ relève de l'INSTANCE et vit dans Administration ; _ce que la collection EST_ relève du CORPUS et vit dans la Bibliothèque.** Le panneau **👥 Collections** (AUTH-3) garde donc les accès, et les dix champs de `CollectionUpdate` — régime de diffusion compris — vont à la Bibliothèque. La phrase se vérifie sur le cas limite qui la teste le mieux : `statut_diffusion` décide de ce qui SORT, ce qui sonne administratif, mais il décrit ce que la collection EST vis-à-vis du dehors et se lit à côté de sa licence et de son embargo — pas à côté de la liste de ses membres. Cohérent avec UX-10, dont Administration porte « ce qui porte sur l'INSTANCE et non sur un album ». **Arbitrée DEUX FOIS le même jour, et la seconde est celle qui vaut** : la première l'a été sans avoir lu `static/administration.js`, donc sans savoir que le panneau éditait déjà le nom — « ce que la collection EST » — et le référent, qui n'est pas « qui entre ». La frontière était contredite dès le premier jour par du code qui marche. Remise à l'équipe avec la mesure, elle a été TENUE : la Bibliothèque reçoit les sept champs manquants **et** récupère le nom, la création, la suppression et le référent ; Administration ne garde que les accès et la vue des comptes. **Renvoi, 2026-09-17 : la frontière change.** Hugo a tranché la décision 2 d'`AUTH-12` en (b) : qui entre dans UNE collection se réglera dans cette collection, dans la Bibliothèque, l'Administration gardant la vue transverse des personnes, des groupes et des accès. Ce qui précède reste l'état du code tant que le déménagement n'est pas fait, dans le même geste que `UX-4` et la lecture de l'annuaire **Troisième mouvement, le 2026-09-23 : `UX-16`.** L'usage a contesté la frontière une fois de plus — l'administrateur traverse maintenant deux écrans là où le propriétaire les traversait avant. La sortie retenue n'est plus un déménagement mais un MODULE monté aux deux endroits, et le principe sera écrit dans `CLAUDE.md` pour que la question cesse de rebasculer. Cette case garde son état du 2026-09-10, comme la fiche le prévoit plus bas.
...
- [x] *Administration → 👥 Accès aux collections* : un refus d'accorder, de changer un niveau ou de retirer un accès s'affiche dans la collection dépliée — sa ligne est sous la ligne d'ajout. Le piège n'y était pas une hypothèse : changer un niveau RECHARGE la liste même sur un refus, donc le 409 du dernier propriétaire s'effaçait aussitôt affiché si on ne le reportait pas dans la collection redessinée (`test_un_refus_d_acces_survit_au_rechargement_de_la_collection`)
  ...
  **Renvoi daté, 2026-09-18 (`6ea6b60`, `AUTH-12` étape 3) : la case n'est pas réécrite, l'écran qu'elle nomme n'existe plus.** Le panneau est dans la Bibliothèque, sous le nom « Qui entre », et il n'y a plus de « niveau » à changer — les droits s'y cochent en actes. **Ce que la case a établi survit pourtant au déménagement, et c'est pourquoi elle vaut d'être gardée** : le piège qu'elle décrit n'était pas lié au panneau mais au RECHARGEMENT, et il se repose à l'identique sur des cases à cocher. Le test qu'elle cite porte le même nom et garde la même charge
...
2026-09-10 et reçoit un renvoi daté, plutôt que d'être réécrite. **Tranché le 2026-09-17 :
elle change**, et la case porte son renvoi.
