...
**Arrêté sur** — 2026-09-11, `043a33a` : le droit d'exporter est posé de bout en bout — la case et la Portee, les quinze portes sous leur cliquet, et l'écran qui ne propose un export qu'à qui peut le faire. Livré sur `dev` le jour même, avec un test rouge qu'on n'avait pas vu : l'égalité exacte de `/api/moi` n'avait pas appris le champ `exporter` du lot 2, et c'est la suite complète de la session voisine qui l'a dit, après la poussée — corrigé par `043a33a`, en ajoutant la clé et non en relâchant l'égalité. Les six cases restées ouvertes sur les termes qu'on ne lit pas ont déménagé dans `AUTH-11` : elles portent sur ce qu'on VOIT d'un terme, pas sur ce qu'on peut SORTIR. **Au déploiement**, la migration v27 ferme l'export à tout non-propriétaire : la case se pose dans le panneau des accès, et il faut la cocher pour qui doit exporter. Et ce déploiement ne se fera pas tout seul : `main` est en v25, et la veille (`deploy/veille-deploiement.sh`) refuse toute mise à jour qui change `SCHEMA_VERSION` — c'est un geste humain.
...
Les six cases restées ouvertes dans cette zone ont déménagé dans `AUTH-11` le 2026-09-11 — les exports de dépôt, l'axe « dimension » du croisement, le filtre par valeur d'attribut, l'index plein texte, la collision de libellés et le marqueur « annotée ». Elles portent sur ce qu'on VOIT d'un terme, pas sur ce qu'on peut SORTIR.
...
de concordance, en cours), `COL-2` (le déménagement du bloc), `AUTH-11` (les termes qu'on
ne lit pas, déménagés d'ici le 2026-09-11).
