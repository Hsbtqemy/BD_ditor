...
**Arrêté sur** — 2026-09-07, `0914094` : **cette fiche a été DÉPASSÉE par les faits, et elle ne le
...
### Le code INFRA-7 désigne DEUX chantiers, et il faut trancher
- [ ] **Le code de cette fiche est employé ailleurs pour un tout autre sujet.** Mesuré le 2026-09-07 : `deploy/deployer.sh` et `pilotage/INFRA-10.md` disent « `deployer.sh` fait bien son travail depuis INFRA-7 » ; `tests/test_verifier_deploiement.py` et `tests/test_version_servie.py` ouvrent sur INFRA-7 pour parler du contrôle de déploiement ; et le commit `87544ba` (2026-09-07) porte « INFRA-7 : le contrôle annonçait "moteurs cassés" pour une sonde qu'il n'avait pas pu poser ». Aucun de ces cinq emplois ne parle de durée de session
