...
- [x] **Le renvoi après réouverture, quand une cible EXISTE** : être renvoyé par l'application (barre d'adresse en `auth…/?rd=…`), saisir le mot de passe, et dire si l'on revient sur la page demandée ou si l'on reste sur le portail. Le 2026-09-13, les deux connexions de `proprio` portaient bien leur cible dans `authentication_logs` (`request_uri` renseignée) — ce que le renvoi a fait ensuite n'a pas été observé, et l'équipe ne s'en souvenait pas. C'est la moitié qui décide s'il y a un défaut ou seulement un portail ouvert à la main. **Une trace y répond peut-être déjà** (relevée le 2026-09-15) : `INFRA-9` consigne le 2026-09-06 que `stagiaire` « entre au mot de passe SEUL et arrive directement dans l'application », alors que la règle `two_factor` des administrateurs existait — le second facteur était donc activé pour tous, et seule la branche AVEC cible mène là. C'était une première connexion et non une réouverture, d'où la case laissée ouverte ; la mesure de la déconnexion (zone « Le portail sans destination ») la tranche au passage. **La recette l'a montré le 2026-09-15**, sur une connexion qui n'était pas davantage une réouverture (visite anonyme, aucune session expirée au journal) : `stagiaire` réussit son premier facteur à 13:53:40.42 avec `request_uri` = la racine de l'application, et `bd-app` sert `GET /` puis `/api/moi` à 13:53:40.58 — le renvoi a eu lieu. Détail dans la section « Le retour avec destination, vu dans les journaux ». **Et de nouveau après déconnexion**, à 14:13:43 (case « La déconnexion ramène à l'application »). **L'EXPIRATION, qui donne son nom à la case, est tranchée par le journal A3 de l'application** : la session de `collectif` expire à 11:40:40 sur un lien profond (`?album=2&planche=123&region=41`), il se reconnecte à 11:40:54 avec ce lien pour destination (`authentication_logs`), et supprime la région `41` — celle du lien — à 11:41:07, treize secondes plus tard (`evenement`). Preuve INDIRECTE, retenue par l'équipe le 2026-09-15 : l'arrivée elle-même ne se relit plus, les journaux Docker de `bd-app` antérieurs à 14:11 ayant disparu avec la recréation du conteneur
- [x] **La réouverture sur un compte de `bd-admins`**, seule question que les comptes `one_factor` ne peuvent pas départager : le portail redemande-t-il le second facteur en plus du mot de passe ? **Oui — relevé le 2026-09-15 dans les journaux de la recette.** Le préalable qui la bloquait le 2026-09-13 (aucun appareil enrôlé) était levé entre-temps : `admin-bd` a un TOTP dans `totp_configurations`. Sa session expire à 11:45:56 (« exceeded configured session inactivity ») ; le mot de passe est accepté à 11:46:00 avec la page demandée pour destination, et Authelia répond aussitôt « requires 2FA, cannot be redirected yet » ; cinq codes sont refusés, le sixième accepté à 11:47:19. Le geste était celui d'une passe de QA et non une mesure faite exprès — retenu par l'équipe. Ce qu'il ne montre pas : l'arrivée sur la page demandée, le premier acte d'`admin-bd` au journal A3 ayant lieu dans l'Administration
