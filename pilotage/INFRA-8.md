...
- [x] **L'arbitrage du facteur est ÉPROUVÉ, pas seulement écrit** — 2026-09-06 au soir, sur les deux versants. `chercheur` (`bd-admins`) se voit demander son second facteur ; `stagiaire` entre au mot de passe seul et arrive directement dans l'application. Cette case n'existait pas : l'arbitrage avait été pris, écrit, déployé — et jamais regardé dans un navigateur. C'est la forme de `BD_REFERENT_NOM`, une fonctionnalité livrée que rien n'obligeait à vérifier là où elle compte
- [x] **Et le vérifier a révélé pourquoi il ne s'appliquait pas** : poussé à 11:16, tiré à 11:20, il n'a pris effet qu'à **18:58**, au premier redémarrage fait pour une autre raison. Authelia lit sa configuration au démarrage du PROCESSUS, et `deployer.sh` ne relançait que le service `app` — il déployait la politique d'accès sans jamais l'appliquer. Corrigé le jour même : le script compare `deploy/authelia/` entre les deux commits et redémarre si elle a changé
