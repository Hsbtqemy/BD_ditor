...
### Rouvert le 2026-09-23 — la Transcription déborde, et l'instrument ne regarde pas là
- [ ] **Le panneau de Transcription déborde sous ~700 px de large ET ~600 px de haut** — mesuré le 2026-09-23 par la passe de revue d'`UX-15`, sur 64 situations (8 largeurs × 8 hauteurs) : **11 débordent**, la barre d'aide passant sous la fenêtre, puis les boutons, puis la zone de saisie qui recouvre la barre d'état (à 375 × 400 : saisie 348, boutons 442, aide 502, pour une fenêtre qui s'arrête à 347). **Ce n'est pas la bulle qui dicte la hauteur** — le crop y est réduit à 0 × 0 : c'est le chrome fixe qui ne tient plus, `#tr-text` portant un `min-height` auquel s'ajoutent les boutons, l'aide et les gouttières. Antérieur à `UX-15`, qui n'a touché aucune ligne d'application, et mesuré identique avec et sans ses mutants. **Deux gardes regardent à côté, et c'est le vrai constat** : les gardes neuves d'`UX-15` jouent 1280 et 900 px de large dans une fenêtre de 800 — les largeurs où rien ne déborde —, et `tests/test_e2e_reflow.py` fixe TOUJOURS `height: 900` et n'entre JAMAIS en mode Transcription. Attendu : le chrome tient dans une fenêtre courte, ou la limite de largeur est écrite ; et l'instrument joue au moins une hauteur basse

- [ ] **Sous 899 px, pendant la Transcription, un bouton de mode n'affiche plus que sa pastille de couleur** — `UX-7` clipe `.mode-label` à cette largeur en écrivant que « le `<kbd>` cesse d'être une glose et devient l'affordance » ; `UX-15` a éteint ce `<kbd>` pendant la Transcription le 2026-09-24 (`feb56ff`), pour ne plus promettre une touche qui écrit sa lettre dans la bulle. Les deux décisions sont justes et leur rencontre ne l'est pas. Ce qui ne se perd PAS, mesuré : le bouton reste cliquable, son nom accessible reste « Navigation », son `title` le porte. Attendu : rendre le nom visible à ces largeurs, ou écrire que la pastille suffit — sachant que le budget de la bande a été mesuré à 320 px, où `.modes` seule fait 488 px
