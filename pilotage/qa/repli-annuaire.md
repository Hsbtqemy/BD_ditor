...
- [ ] **Où est la clé ?** — et la réponse la plus probable est la pire. Jusqu'au 2026-09-07, trois fichiers du dépôt documentaient `crypto hash generate --password '…'`, qui écrit le mot de passe en clair dans `~/.bash_history`. Les comptes concernés sont ceux d'AVANT la bascule, c'est-à-dire exactement ceux dont ce fichier de repli porte les condensés. Attendu : `grep -c -- "--password" ~/.bash_history` rend 0. **S'il rend autre chose, la case suivante est cochée et le repli est compromis du même geste** — on a trouvé la clé, elle est lisible par quiconque obtient le compte
- [ ] **La rotation, si elle a lieu, se fait à DEUX endroits.** Changer le mot de passe dans LLDAP ne ferme rien ici : le condensé de `users_database.yml` reste celui du mot de passe exposé, et basculer sur le repli le RÉADMET. Attendu : le condensé est regénéré dans le fichier aussi. Sans quoi la rotation ne tient que tant qu'on ne se sert pas du recours — c'est-à-dire tant que tout va bien. Et `verifier_comptes.py` ne dira rien : il approuve une syntaxe, pas une fraîcheur
...
- [ ] **Le compte du fichier entre pour de bon** — jusqu'à l'application, pas jusqu'au portail. Attendu : `bd.edito-revue.fr` s'ouvre sur la Bibliothèque, avec du corpus visible
