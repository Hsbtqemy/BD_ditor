...
- [ ] **La veille de déploiement est arrêtée** (`sudo systemctl stop bd-deploiement.timer`), et l'heure est notée. `deployer.sh` refuse un arbre sale, et la passe salit exprès un fichier suivi par git : sans cet arrêt, un tir de la veille écrit un témoin d'échec et l'unité passe `failed` pour une raison qui n'en est pas une
...
- [ ] La veille est relancée, et **son tir suivant est vert** (`systemctl status bd-deploiement`). Une veille laissée arrêtée est le pire résidu possible de cette passe : l'instance cesse de se déployer et rien ne le dit — le déploiement continue de « marcher » en ne faisant plus rien
...
- [ ] `users_database.example.yml` fait générer un condensé avec `authelia/authelia:4.38` alors que l'instance sert 4.39.22. Vérifier si la commande de cette version répond encore : c'est là qu'on ira le jour où il faudra ajouter un compte au repli
